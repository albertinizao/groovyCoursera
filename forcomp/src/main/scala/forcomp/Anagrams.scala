...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val ocurrence : List[Occurrences] = occurrences.map( x => (for(i <- 1 until (x._2+1)) yield (x._1,i)).toList)
    ocurrence.foldRight(List[Occurrences](Nil))((x,y) => y ++ (for(i <- x; j <- y) yield (i :: j)))
  }
  ...
  def sentenceAnagramsInner(ocur: Occurrences): List[Sentence] = {
    if (ocur.isEmpty) {
      List(Nil)
    } else {
      val combs = combinations(ocur)
      for (i <- combs if dictionaryByOccurrences.keySet(i);
           j <- dictionaryByOccurrences(i);
           s <- sentenceAnagramsInner(subtract(ocur, i))) yield {j :: s}
    }
  }
}
